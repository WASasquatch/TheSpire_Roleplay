...
export async function registerOverlookRoutes(app: FastifyInstance, db: Db): Promise<void> {
  ...
  app.get<{ Querystring: { u?: string } }>("/overlook/image", async (req, reply) => {
    ...
    /**
     * Fetch, chasing redirects OURSELVES so every hop gets vetted.
     *
     * `redirect: "follow"` would be an SSRF hole: we would validate the first
     * hostname and then let undici quietly follow a 302 to 169.254.169.254.
     * `redirect: "error"` closes that hole but breaks most real image hosts,
     * which redirect as a matter of course (CDN edges, signed-URL handoffs,
     * shortlinks). So: manual, one hop at a time, re-running the protocol and
     * address checks on each new URL.
     */
    const MAX_HOPS = 4;
