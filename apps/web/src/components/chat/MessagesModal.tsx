...
// (about a quarter of a 1100px modal). Persisted to localStorage so a
// user's drag-resize sticks across reopens (clamped into range on load).
// Mobile ignores this entirely — the left pane goes full-width via flex-1
// when it's the only visible pane.
...
const listWidthDim = createPersistedDimension({
  key: LIST_WIDTH_STORAGE_KEY,
  min: LIST_WIDTH_MIN,
  max: LIST_WIDTH_MAX,
  default: LIST_WIDTH_DEFAULT,
  outOfRange: "clamp",
  ssrGuard: true,
});
...
export function MessagesModal({ onClose, onCommand, initialOtherUserId, initialOtherCharacterId, onOpenProfile }: Props) {
  ...
  const [listWidth, setListWidth] = useState<number>(listWidthDim.load);
  useEffect(() => {
    listWidthDim.save(listWidth);
  }, [listWidth]);
   ...
   */
  // `identityKey` (composite in-memory key for a (userId, characterId)
  // pair, OOC/master = empty characterId) lives in ../lib/identity.
  ...
  useEffect(() => {
    ...
    if (selectedTarget !== null) {
      const conv = Object.values(useChat.getState().dmConversations)
        .find((c) =>
          identityEquals(c.otherUserId, c.otherCharacterId, selectedTarget.userId, selectedTarget.characterId),
        );
      if (conv) {
                  ...
                  <ul className="space-y-1">
                    {friendRows.map((row) => (
                      <UserRow
                        ...
                        row={row}
                        active={
                          identityEquals(selectedTarget?.userId, selectedTarget?.characterId, row.userId, row.characterId)
                        }
          ...
          >
            {selectedTarget ? (() => {
              ...
              const matchesTarget = (r: { userId: string; characterId: string | null }) =>
                identityEquals(r.userId, r.characterId, selectedTarget.userId, selectedTarget.characterId);
              const selectedRow =
                friendRows.find(matchesTarget)
                  ?? nonFriendConvRows.find(matchesTarget)
                  ?? (composeFallback
                      && identityEquals(composeFallback.userId, composeFallback.characterId, selectedTarget.userId, selectedTarget.characterId)
                      ? {
...
function ThreadPane({
  ...
  const myAvatarUrl = useChat((s) => {
    ...
    for (const list of Object.values(s.occupants)) {
      const row = list.find((o) => identityEquals(o.userId, o.characterId, meId, matchChar));
