...
export function WorldEntitiesTab({
  ...
  const kinds: WorldEntityKindDef[] = useMemo(() => {
    ...
    return [...BUILTIN_ENTITY_KINDS, ...custom];
  }, [detail.entityKinds]);
  ...
  async function removeActiveKind() {
    ...
    if (!window.confirm(`Delete the "${activeDef?.label}" kind? (only works when it has no entries)`)) return;
    setErr(null);
    try {
      ...
      {addingKind ? (
        ...
        <div className="space-y-2">
          ...
          {entriesOfKind.length === 0 ? (
            <p className="italic text-keep-muted">No entries of this kind yet.</p>
          ) : (
              ...
              {entriesOfKind.map((e: WorldEntityLight) => (
                <li key={e.id}>
                  ...
                  >
                    ...
                    {!e.isPublic ? <span className="shrink-0 text-[10px] uppercase tracking-widest text-keep-muted">hidden</span> : null}
                  </button>
                </li>
              ))}
            </ul>
  ...
  return (
    <div className="space-y-2 rounded border border-keep-rule/60 bg-keep-bg/30 p-3 text-sm">
      <label className="block">
        <span className="text-[11px] uppercase tracking-widest text-keep-muted">Name</span>
      ...
      <label className="block">
        <span className="text-[11px] uppercase tracking-widest text-keep-muted">Slug</span>
      ...
      <label className="block">
        <span className="text-[11px] uppercase tracking-widest text-keep-muted">Summary</span>
        <input value={summary} onChange={(e) => setSummary(e.target.value)} className="mt-0.5 w-full rounded border border-keep-rule bg-keep-bg px-2 py-1" placeholder="One-line description" />
      </label>
      <div className="block">
        <div className="mb-0.5 flex items-center justify-between">
          <span className="text-[11px] uppercase tracking-widest text-keep-muted">Body</span>
          <EntryLinkPicker targets={targets} onPick={insertToken} />
        </div>
        <textarea ref={bodyRef} value={bodyHtml} onChange={(e) => setBodyHtml(e.target.value)} rows={8} className="w-full rounded border border-keep-rule bg-keep-bg px-2 py-1 font-mono text-xs" placeholder="HTML allowed. Link entries with @npc:slug, @location:slug, @lore:slug …" />
      </div>
      <StatsEditor stats={stats} onChange={setStats} />
      <label className="block">
        <span className="text-[11px] uppercase tracking-widest text-keep-muted">Tags (comma-separated)</span>
        <input value={tagsText} onChange={(e) => setTagsText(e.target.value)} className="mt-0.5 w-full rounded border border-keep-rule bg-keep-bg px-2 py-1" placeholder="e.g. capital, port, contested" />
      </label>
      <label className="block">
        <span className="text-[11px] uppercase tracking-widest text-keep-muted">Image URL</span>
        <input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className="mt-0.5 w-full rounded border border-keep-rule bg-keep-bg px-2 py-1" placeholder="https://…" />
      </label>
      <label className="flex items-center gap-2">
        <input type="checkbox" checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} />
        <span className="text-xs">Public (visible to non-editors)</span>
      </label>
      <div className="flex items-center gap-2 pt-1">
        <button type="button" disabled={busy || !name.trim()} onClick={() => void save()} className="rounded border border-keep-action bg-keep-action/15 px-3 py-1 text-xs text-keep-action disabled:opacity-50">Save</button>
        ...
        {entityId ? <button type="button" disabled={busy} onClick={() => void remove()} className="ml-auto rounded border border-keep-accent/50 px-3 py-1 text-xs text-keep-accent hover:bg-keep-accent/10">Delete</button> : null}
      </div>
    </div>
  );
}
...
function StatsEditor({ stats, onChange }: { stats: Array<[string, string]>; onChange: (s: Array<[string, string]>) => void }) {
  return (
    <div className="space-y-1">
      <span className="text-[11px] uppercase tracking-widest text-keep-muted">Stats (optional)</span>
      {stats.map(([k, v], i) => (
        <div key={i} className="flex items-center gap-1">
          <input value={k} onChange={(e) => { const n = [...stats]; n[i] = [e.target.value, v]; onChange(n); }} placeholder="key" className="w-1/3 rounded border border-keep-rule bg-keep-bg px-2 py-1 text-xs" />
          <input value={v} onChange={(e) => { const n = [...stats]; n[i] = [k, e.target.value]; onChange(n); }} placeholder="value" className="flex-1 rounded border border-keep-rule bg-keep-bg px-2 py-1 text-xs" />
          <button type="button" onClick={() => onChange(stats.filter((_, j) => j !== i))} className="rounded border border-keep-rule px-1.5 text-xs text-keep-muted hover:bg-keep-banner" title="Remove">✕</button>
        </div>
      ))}
      <button type="button" onClick={() => onChange([...stats, ["", ""]])} className="rounded border border-keep-rule px-2 py-0.5 text-[11px] text-keep-muted hover:bg-keep-banner">+ Stat</button>
    </div>
  );
}
...
function NewKindForm({
  ...
  async function create() {
    ...
    try {
      ...
      onError(x instanceof Error ? x.message : "create failed");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-2 rounded border border-keep-rule/60 bg-keep-bg/30 p-3 text-sm">
      <p className="text-[11px] uppercase tracking-widest text-keep-muted">New custom kind</p>
      <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Label (e.g. Deity)" className="w-full rounded border border-keep-rule bg-keep-bg px-2 py-1" />
      <input value={effectiveKey} onChange={(e) => { setKey(e.target.value); setKeyDirty(true); }} placeholder="key" className="w-full rounded border border-keep-rule bg-keep-bg px-2 py-1 font-mono text-xs" />
      <input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short description" className="w-full rounded border border-keep-rule bg-keep-bg px-2 py-1" />
      <input value={icon} onChange={(e) => setIcon(e.target.value)} placeholder="Icon (emoji)" className="w-24 rounded border border-keep-rule bg-keep-bg px-2 py-1" maxLength={4} />
      <div className="flex items-center gap-2">
        <button type="button" disabled={busy || !label.trim()} onClick={() => void create()} className="rounded border border-keep-action bg-keep-action/15 px-3 py-1 text-xs text-keep-action disabled:opacity-50">Create</button>
        <button type="button" onClick={onCancel} className="rounded border border-keep-rule px-3 py-1 text-xs hover:bg-keep-banner">Cancel</button>
      </div>
    </div>
  );
