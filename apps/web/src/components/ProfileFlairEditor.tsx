...
export function ProfileFlairEditor({ characterId }: { characterId: string | null }) {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const r = await fetch(buildUrl(characterId), { credentials: "include" });
...
function buildUrl(characterId: string | null): string {
  return characterId
    ? `/me/profile-flair?characterId=${encodeURIComponent(characterId)}`
    : "/me/profile-flair";
}
