...
async function loadRankTierLookup(
  db: Db,
...
interface TransitionCatalogRow {
  key: string;
  label: string;
  description: string;
  cost: number;
 ...
 */
async function buildTransitionCatalog(db: Db, serverId: string): Promise<TransitionCatalogRow[]> {
  const rows = await db
    .select()
 ...
 */
async function resolveTransitionForServer(
  db: Db,
  serverId: string,
  key: string,
