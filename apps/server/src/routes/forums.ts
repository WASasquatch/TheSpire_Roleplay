...
import { forumBans, forumCreationApplications, forumMembers, forumMembershipApplications, forums, messages, rooms, users, worlds } from "../db/schema.js";
...
export async function registerForumRoutes(app: FastifyInstance, db: Db, io: Io, uploadsRoot: string): Promise<void> {
  ...
  app.get<{ Params: { id: string } }>("/forums/:id/roles", async (req, reply) => {
    ...
    const mods = await db
      .select({ userId: forumMembers.userId, username: users.username, since: forumMembers.joinedAt })
      .from(forumMembers)
      .leftJoin(users, eq(users.id, forumMembers.userId))
      .where(and(eq(forumMembers.forumId, gate.forum.id), eq(forumMembers.role, "mod")));
    return {
      owner: { userId: gate.forum.ownerUserId, username: owner?.username ?? "unknown" },
      mods: mods.map((m) => ({ userId: m.userId, username: m.username ?? "unknown", since: +m.since })),
    };
  });

  const modBody = z.object({ target: z.string().trim().min(1).max(120) }).strict();
  ...
  app.put<{ Params: { id: string }; Body: unknown }>("/forums/:id/mods", async (req, reply) => {
    ...
    await db.insert(forumMembers)
      .values({ forumId: gate.forum.id, userId: target.userId, role: "mod" })
      .onConflictDoUpdate({
  ...
  /** Owner-or-mod gate for the review surfaces (matrix: mods review
   *  membership applications; everything else stays owner-only). */
  async function requireForumMod(req: Parameters<typeof getSessionUser>[0], forumId: string) {
    const me = await getSessionUser(req, db);
    if (!me) return { fail: { code: 401 as const, error: "auth" } };
    const a = await forumAuthority(db, me, forumId);
    if (!a.forum) return { fail: { code: 404 as const, error: "no forum" } };
    if (!a.isMod) return { fail: { code: 403 as const, error: "forum owner or mod only" } };
    return { me, forum: a.forum };
  }
  ...
  app.get<{ Params: { id: string } }>("/forums/:id/membership-applications", async (req, reply) => {
    const gate = await requireForumMod(req, req.params.id);
    if ("fail" in gate) { reply.code(gate.fail.code); return { error: gate.fail.error }; }
